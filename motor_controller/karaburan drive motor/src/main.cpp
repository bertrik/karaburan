...
via H bridge  IBT-4 module  powered separately, control pins being:
IN1 --> pin3
IN2 --> pin5 //pin11 not supported on nano every
*/

//serial via USB or Rx0 Tx0
//Rx0 pin
//Tx0 pin
/*
...
const int motorAPin1 = 9;//pin 9 on boat; // Timer1 output A
const int motorAPin2 = 10;//pin 10 on boat; // Timer1 output B
const int motorBPin1 = 3;//pin 3 on boat;
const int motorBPin2 = 11;//pin 5 on proto //pin 11 on boat;

const int trappeSigPin = 8; // D8
const int relayPin = 17; // = A3
...
const int targetFrequency = 4000; // 2kHz recommended... this has no effect...
...
int maxPWMA = 100; // 50% of 255
int maxPWMB = 100; // 50% of 255
int pwm_tmp = 0;
...
//boost variables
bool motorA_boosting = false;
bool motorB_boosting = false;
unsigned long motorA_boost_endtime = 0;
unsigned long motorB_boost_endtime = 0;
const int BOOST_PWM_VALUE = 180; // PWM during boost
const unsigned long BOOST_DURATION_MS = 200; // Boost time in milliseconds
const int START_THRESHOLD = 70; // PWM threshold below which boost is needed
...
void setupTimer1PWM() {
  ...
  // Set TOP value for target frequency (based on 16MHz crystal)
...
void setupTimer2() {
    // --- Configure Timer2 for 1ms Interrupt (CTC mode) ---
  ...
  // WGM21 in TCCR2A for CTC mode
  TCCR2A |= (1 << WGM21); 

  // Set prescaler to 64 and start timer
  // CS22 and CS20 for prescaler 64
  TCCR2B |= (1 << CS22) | (1 << CS20); 

  // Set Output Compare Register A (OCR2A) for 1ms interrupt
  // OCR2A = (F_CPU / (prescaler * desired_frequency_Hz)) - 1
  // OCR2A = (16,000,000 / (64 * 1000)) - 1 = 250 - 1 = 249
  OCR2A = 124; 

  // Enable Timer2 Compare Match A Interrupt
  // OCIE2A in TIMSK2 (Timer Interrupt Mask Register)
  TIMSK2 |= (1 << OCIE2A); 
}


// Interrupt Service Routine (ISR) for Timer2 Compare Match A - every 1 ms
ISR(TIMER2_COMPA_vect) {
  // Keep this ISR as short and efficient as possible!
  if(digitalRead(trappeSigPin)==LOW){
    if (old_sig == LOW){ //increase every clock if still low
      trappeDur++;
    }else{ //reset counter at falling edge
      trappeDur=0;
    }
    old_sig=LOW;
  }else{
  ...
  // Avoid Serial.print() or complex operations inside ISRs.
}
...
void setup() {
  ...
  pinMode(trappeSigPin, INPUT_PULLUP);
  pinMode(relayPin, OUTPUT);
  digitalWrite(relayPin, LOW); //control of relay (high=relay open NC==> motor controlled by boat)
  ...
  compass.setCalibrationOffsets(538,176,156);
  compass.setCalibrationScales(1.34,1.25,0.69);
...
//chat gpt variant with boost
void updatePWM() {
    unsigned long now = millis();
    
    // Check if boost needed for motor A
    if (!motorA_boosting && pwmA != 0 && abs(pwmA) < START_THRESHOLD && internal_pwmA == 0) {
        motorA_boosting = true;
        motorA_boost_endtime = now + BOOST_DURATION_MS;
    }
    // Check if boost ended for motor A
    if (motorA_boosting && now >= motorA_boost_endtime) {
        motorA_boosting = false;
    }

    // Check if boost needed for motor B
    if (!motorB_boosting && pwmB != 0 && abs(pwmB) < START_THRESHOLD && internal_pwmB == 0) {
        motorB_boosting = true;
        motorB_boost_endtime = now + BOOST_DURATION_MS;
    }
    // Check if boost ended for motor B
    if (motorB_boosting && now >= motorB_boost_endtime) {
        motorB_boosting = false;
    }

    int targetA = motorA_boosting ? (pwmA > 0 ? BOOST_PWM_VALUE : -BOOST_PWM_VALUE) : pwmA;
    int targetB = motorB_boosting ? (pwmB > 0 ? BOOST_PWM_VALUE : -BOOST_PWM_VALUE) : pwmB;

    //int deltaA = sgn(targetA - internal_pwmA)*8; //add +/- 16 so that max steps are reached in 20x100ms = 2sec
    //int deltaB = sgn(targetB - internal_pwmB)*8;//add +/- 16
      
    //internal_pwmA = constrain(internal_pwmA + deltaA, -abs(targetA), abs(targetA));
    //internal_pwmB = constrain(internal_pwmB + deltaB, -abs(targetB), abs(targetB));

    internal_pwmA = targetA;
    internal_pwmB = targetB;
    ...
    /*
    if (internal_pwmA < 0) {//backward
        analogWrite(motorAPin2, 255); //TA6586 pin2 (FI)
        analogWrite(motorAPin1, 255-internal_pwmA); //TA6586 pin1 (BI)
    } else { //forward
        analogWrite(motorAPin2, 255-internal_pwmA);//TA6586 pin2 (FI)
        analogWrite(motorAPin1, 255); //TA6586 pin1 (BI)     
    }*/
    ...
    /*
    if (internal_pwmB < 0) {//backward
        analogWrite(motorBPin2, 255); //TA6586 pin2 (FI)
        analogWrite(motorBPin1, 255-internal_pwmB); //TA6586 pin1 (BI)
    } else { //forward
        analogWrite(motorBPin2, 255-internal_pwmB);//TA6586 pin2 (FI)
        analogWrite(motorBPin1, 255); //TA6586 pin1 (BI)     
    }*/


    if (trappeDur >= 2){
      Serial.println("RC detected");
      digitalWrite(relayPin, HIGH); // gibe hand to boat and its RC for 60 seconds.
      delay(60000);
      Serial.println("got back control.");
      digitalWrite(relayPin, LOW); // gibe hand to boat and its RC for 60 seconds.
    }
}
...
FloatArray calibrateCompass(){
  ...
  speedCommand=128;//128;//max speed
  ...
  for (int i=0;i<27;i++){
    updatePWM();
    delay(10);//wait 10ms to reduce current peak and smoothly change pwm
  }
...
void handlePWMcommand(String command) {
  if (motor_enable == true) {
    ...
    String debuginfo = command.substring(space3 + 1);
...
void loop() {
  ...
  if (currentMillis - previousMillis >= 50) {//every 50ms 
    ...
    if (auto_nav==true && (counter%4)==0){//loop of 200ms second
      // Call the function to perform actions every 1 second
      navigateOnCompass();
    }
    setPWMtarget();
    updatePWM(); 
  ...
  */
  ...
  if (Serial.available() > 0) {
    ...
    if (command.startsWith("POST PWM")) {
      ...
      handleCALIBcommand();
    } else if (command.startsWith("GET PWM")) {
      returnPWMcommand();
    } else if (command.startsWith("GET SPEED")) {
      returnSPEEDcommand();
    } else if (command.startsWith("GET NAV")) {
      returnNAVcommand();
    } else if (command.startsWith("GET CALIB")) {
      returnCALIBcommand();
    } else if (command.startsWith("GET HEADING")) {
      returnHEADINGcommand();
    } else if (command.startsWith("GET WD")) {
      returnWDcommand();
    }*/
  }
